<script>
    ...
    let scene, camera, renderer, objLoader, mtlLoader, gltfLoader, controls, currentModel, composer, mixer, exploded = false, isGLTF=false;
    let distanceValue = 0;
    ...
    selectBox.addEventListener("change", async (event) => {
        ...
        await cleanAndLoadItem(selectedItem);
    });
    ...
    async function cleanAndLoadItem(selectedItem) {
        if (currentModel && currentModel.model) {
            ...
            if (nullObject) {
                nullObject.remove(currentModel.model);
            }
        }
    ...
    async function loadAndGroupModels(objectsUrls, texturesUrls, mtlUrls) {
        ...
        for (let i = 0; i < urls.length; i++) {
            ...
            try {
                const model = await loadModel(url, fileType, textures);
    ...
    async function loadModel(file, fileType, texturesUrls) {
    ...
    function waitForModelReady() {
        const intervalId = setInterval(() => {
            if (currentModel && currentModel.model && currentModel.model.geometry && currentModel.model.material) {
    ...
    async function LoadStep(fileUrl) {
        ...
        for (let i = 0; i < result.meshes.length; i++) {
            ...
            if (resultMesh.color && resultMesh.color.length === 3) {
                ...
                color = new THREE.Color(r / 255, g / 255, b / 255);
            } else if (resultMesh.attributes.color && resultMesh.attributes.color.array) {
                const colorArray = resultMesh.attributes.color.array;
                const r = colorArray.reduce((sum, _, idx) => idx % 3 === 0 ? sum + colorArray[idx] : sum, 0) / (colorArray.length / 3);
                const g = colorArray.reduce((sum, _, idx) => idx % 3 === 1 ? sum + colorArray[idx] : sum, 0) / (colorArray.length / 3);
                const b = colorArray.reduce((sum, _, idx) => idx % 3 === 2 ? sum + colorArray[idx] : sum, 0) / (colorArray.length / 3);
            ...
            const materialKey = `${color.getHex()}_${opacity}`;
    ...
    function createMaterialTextures(materialName, textureUrls) {
        const textures = {};
        
        for (const [mapType, names] of Object.entries(PBR_MAPS)) {
            for (const name of names) {
            const regex = new RegExp(`${materialName}.*${name}.*`, 'i');
            const matchingUrl = textureUrls.find(url => regex.test(url));
        
            if (matchingUrl) {
                textures[mapType] = new THREE.TextureLoader().load(matchingUrl);
                break;
            }
            }
        }
    ...
    function hasExistingTextures(material) {
        for (const mapType of Object.keys(PBR_MAPS)) {
            if (material[mapType]) {
            return true;
            }
        }
    ...
    function applyMaterialToMeshModel(mesh, textureUrls) {
        mesh.traverse((child) => {
            if (child.isMesh) {
                if (child.material && hasExistingTextures(child.material)) {
                    return;
                }
            
                if (child.material && child.material.name) {
                    const materialName = child.material.name.toLowerCase();
                    const materialTextures = createMaterialTextures(materialName, textureUrls);
            
                    if (Object.keys(materialTextures).length > 0) {
                    const material = new THREE.MeshStandardMaterial();
            
                    for (const [mapType, texture] of Object.entries(materialTextures)) {
                ...
                } else if (child.geometry && child.geometry.hasAttribute('color')) {
                    child.material = new THREE.MeshStandardMaterial({
                    vertexColors: true
                    });
                } else if (child.userData && child.userData.color) {
                    const color = new THREE.Color(child.userData.color);
